...
interface MentimeterParticipantViewProps {
...
export function MentimeterParticipantView({
  presentationId,
  roomCode,
  userName,
  userId,
  ...
  onResponseSubmitted,
  debug = false
}: MentimeterParticipantViewProps) {
  ...
  const {
    ...
    eventHandlers: {
      onRoomJoined: (data) => {
        ...
        toast.success(`Joined presentation room ${data.roomCode}`)
      },
      onParticipantJoined: (data) => {
        console.log('👤 Participant joined:', data)
        setParticipantCount(data.participantCount)
        toast.info(`${data.userName} joined the presentation`)
      },
      onParticipantLeft: (data) => {
        console.log('👤 Participant left:', data)
        setParticipantCount(data.participantCount)
        toast.info(`${data.userName} left the presentation`)
      },
      onSlideChanged: (data) => {
        ...
        setSelectedOptions([])
        setTimeLeft(null)
        toast.info(`Slide changed to ${data.slideIndex + 1}`)
      },
      ...
      onResponseSubmitted: (data) => {
        ...
        if (data.userName !== userName) {
          toast.info(`${data.userName} submitted a response`)
        }
      },
      onVotesUpdated: (data) => {
        console.log('🗳️ Votes updated:', data)
        if (showResults) {
          toast.info(`Votes updated: ${data.votes.length} options`)
        }
      },
      onError: (data) => {
      ...
      onConnect: () => {
        ...
        toast.success('Connected to presentation server')
      },
      onDisconnect: () => {
        console.log('🔌 Disconnected from presentation server')
        toast.error('Disconnected from presentation server')
      }
    }
  })
  ...
  const handleWordCloudResponse = () => {
    ...
    toast.success('Words submitted successfully!')
  }
  ...
  const renderResponseForm = () => {
    ...
    switch (slideType) {
      ...
      case 'question_only':
        return (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">This is a question-only slide. No response needed.</p>
          </div>
        )
      ...
      default:
        return (
          <div className="text-center py-8">
            <p className="text-muted-foreground">No response form available for this slide type.</p>
          </div>
        )
    }
  }
